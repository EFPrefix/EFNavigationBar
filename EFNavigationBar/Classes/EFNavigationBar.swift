...
public extension UINavigationBar {
    ...
    fileprivate func ef_setBackgroundImage(image: UIImage) {
    ...
    fileprivate func ef_setBackgroundColor(color: UIColor) {
    ...
    fileprivate func ef_setBackgroundAlpha(alpha: CGFloat) {
...
public extension UINavigationController {
    ...
    fileprivate func updateNavigationBar(fromVC: UIViewController?, toVC: UIViewController?, progress: CGFloat) {
        ...
        let newBarTintColor  = EFNavigationBar.middleColor(fromColor: fromBarTintColor, toColor: toBarTintColor, percent: progress)
        ...
        let newTintColor = EFNavigationBar.middleColor(fromColor: fromTintColor, toColor: toTintColor, percent: progress)
        ...
        //        let newTitleColor = EFNavigationBar.middleColor(fromColor: fromTitleColor, toColor: toTitleColor, percent: progress)
        ...
        let newBarBackgroundAlpha = EFNavigationBar.middleAlpha(fromAlpha: fromBarBackgroundAlpha, toAlpha: toBarBackgroundAlpha, percent: progress)
...
class NothingToSeeHere {
    static func harmlessFunction(){
        ...
        //            (types[index] as? EFAwakeProtocol.Type)?.efAwake() //如果该类实现了SelfAware协议，那么调用 awake 方法
        //            (types[index] as? EFFatherAwakeProtocol.Type)?.fatherAwake()
        //        }
        //        types.deallocate(capacity: typeCount)
        UINavigationBar.efAwake()
        UIViewController.efAwake()
...
// 3. 自定义类实现 EFAwakeProtocol 协议，重写 efAwake 方法
//    自定义类实现 EFFatherAwakeProtocol 协议，重写 fatherAwake 方法
