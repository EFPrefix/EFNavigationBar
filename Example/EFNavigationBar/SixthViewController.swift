...
extension SixthViewController {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        tableView.delegate = nil
        navigationController?.navigationBar.ef_setTranslationY(translationY: 0)
    }
    ...
    func setNavigationBarTransformProgress(progress: CGFloat) {
        navigationController?.navigationBar.ef_setTranslationY(translationY: -CGFloat(kNavBarHeight) * progress)
        // 没有系统返回按钮，所以 hasSystemBackIndicator = false
        // 如果这里不设置为false，你会发现，导航栏无缘无故多出来一个返回按钮
        navigationController?.navigationBar.ef_setBarButtonItemsAlpha(alpha: 1 - progress, hasSystemBackIndicator: false)
    }
}
